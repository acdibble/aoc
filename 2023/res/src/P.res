...
let sepBy1 = (parser, sep) => Parser(
  input =>
    switch run(parser, input) {
    | Ok((value, rest)) =>
      many(keepRight(sep, parser))->map(v => List.concat(list{value}, v))->run(rest)
    | Error(_) as error => error
    },
)

let sepBy = (parser, sep) => sepBy1(opt(parser), sep)->map(v => List.filterMap(v, x => x))
...
let digit = "0123456789"->String.split("")->anyOf

let integer = many1(digit)->map(v => List.toArray(v))->map(v => Array.join(v, ""))
