...
const repairRobot = async (codes) => {
  ...
  const handleOutput = async (output) => {
    ...
    map[location.y][location.x] = oxygenFound ? 'O' : ' ';
    location[xOrY] += sign;
...
(async () => {
  ...
  let pertinentLines = map.filter((line) => !line.every((el) => el === 'X'));
  ...
  while (changeMade) {
    ...
    pertinentLines = pertinentLines.map((line, i) => line.map((col, j) => {
      if (col === 'D') return ' ';
      if (col !== ' ') return col;
      const prevLine = pertinentLines[i - 1];
      const nextLine = pertinentLines[i + 1];
      if (
        (prevLine && prevLine[j] === 'O')
        || (nextLine && nextLine[j] === 'O')
        || (line[j - 1] === 'O' || line[j + 1] === 'O')
      ) {
        changeMade = true;
        return 'O';
      }
